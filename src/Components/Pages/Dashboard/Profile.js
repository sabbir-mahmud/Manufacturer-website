...
const Profile = () => {
  ...
  const { data: userDetails, loading: dataLoading } = useQuery(user.email, () =>
    fetch(`http://localhost:5000/api/users/sabbir.mahmud.zim@gmail.com`, {
      method: "GET",
      headers: {
  ...
  return (
    <div className="my-24 container mx-auto px-14">
      ...
      <div className="profile flex flex-col lg:flex-row items-center">
        <div className="avatar mr-14">
          <div className="w-48 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
            <img
              src={userDetails[0]?.avatar ? userDetails[0].avatar : img}
              alt=""
            />
          </div>
        </div>
        <div className="user-details my-9 lg:my-0">
          ...
          <p>
            Bio: {userDetails[0]?.bio ? userDetails[0]?.bio : "Web Developer"}
          </p>
        </div>
      </div>
      <div className="mb-5 mt-14">
        ...
        <p className="mb-2 font-serif text-md text-primary ml-3">
          phone:{" "}
          {userDetails[0]?.phone ? userDetails[0]?.phone : "+8801756644241"}
        </p>
        <p className="mb-2 font-serif text-md text-primary ml-3">
          address:{" "}
